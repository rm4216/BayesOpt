...
from Baselines.chemvae.tgru_k2_gpu import TerminalGRU
...
def encoder_model(params):
    ...
    x_in = Input(shape=(params['MAX_LEN'], 1), name='input')
...
def decoder_model(params):
    ...
    true_seq_in = Input(shape=(params['MAX_LEN'],),
    ...
    #                     )(z_reps)
    x_out = Dense(int(params['MAX_LEN']),
              activation=params['activation'],
              name="decoder_dense_last")(z)
    if params['dropout_rate_mid'] > 0:
        x_out = Dropout(params['dropout_rate_mid'])(x_out)
    if params['batchnorm_mid']:
        x_out = BatchNormalization(axis=-1,
                               name="decoder_dense_last_norm")(x_out)
...
def property_predictor_model(params):
    ...
    if ('reg_prop_tasks' in params) and (len(params['reg_prop_tasks']) > 0):
        reg_prop_pred = Dense(1, activation='linear',
