function testElectronSphere6np_Parallel(dim, loss, experiment)
...
%     for experiment=1:20
    rng(experiment)
    [model, Xopt_i] = rembo(total_iter, dim, high_dim, rotate, 1, 1, experiment, loss);
    Xopt = Xopt_i;
    yopt = model.f;
%     end
    if strcmp(loss,'lcb')
        string_out = sprintf('/homes/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/BayesOpt/Baselines/rembo/demos/ElectronShpere6np/ElectronSphere6np_lcb_d=%d_seed_%d', dim, experiment);
    elseif strcmp(loss,'Neg_ei')
        string_out = sprintf('/homes/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/BayesOpt/Baselines/rembo/demos/ElectronShpere6np/ElectronSphere6np_Neg_ei_d=%d_seed_%d', dim, experiment);
    elseif strcmp(loss,'Neg_pi')
        string_out = sprintf('/homes/rm4216/Desktop/ImperialCollege/Python/Github_manifold_bo/BayesOpt/Baselines/rembo/demos/ElectronShpere6np/ElectronSphere6np_Neg_pi_d=%d_seed_%d', dim, experiment);
    end
...
%         init_f = obj_fct(init_pt);                     % Evaluate initial point.
        ...
        % Do optimization.
        model = sparse_opt(obj_fct, total_iter-1, model, loss);
