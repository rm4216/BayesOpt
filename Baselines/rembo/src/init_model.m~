...
elseif strcmp(cov_model,'M52ard')
    model.cov_model = @(hyp, x, z, i)covMaternard(5, hyp, x, z);
...
model.hyp = hyp;
model.bounds = bounds;
model.d = d;    % dimensionality of feature space

model.cur_hyp = hyp(1);
model.exploit_count = 0;
% model.hyper_bound = log([0.01, 50]);
model.hyper_bound = log(repmat([0.01, 50], d, 1));
